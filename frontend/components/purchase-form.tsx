...
console.log(deployedContract.abi);
console.log(deployedContract.address);
  const calls = useCallback(()=>{     
      ...
      return [contract.populate(Functions.purchaseGiftCard, [currentToken, currentAmount])]
  }, [deployedContract, contract,currentAmount, currentToken]);
  const {send, sendAsync, data, error, isPending} = useSendTransaction({
    calls: calls()
  });
  const {data: waitData, status: waitStatus, error: waitError, isLoading: waitIsLoading, isError: waitIsError} = useTransactionReceipt({
    watch: true,
    ...
    <>
      <Card className="border border-purple-800/30 bg-card">
        <CardHeader>
          ...
          <CardDescription>Choose a token and amount to purchase a new NFT gift card.</CardDescription>
        </CardHeader>
        <CardContent>
          <Form {...form}>
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <Select onValueChange={field.onChange} defaultValue={field.value}>
                      ...
                      <SelectContent>
                        <SelectItem value="0x049d36570d4e46f48e99674bd3fcc84644ddd6b96f7c741b1562b82f9e004dc7">Ethereum (ETH)</SelectItem>
                        <SelectItem value="0x04718f5a0fc34cc1af16a1cdee98ffb20c31f5cd61d6ab07201858f4287c938d">Starknet (STRK)</SelectItem>
                      </SelectContent>
                    </Select>
                    <FormDescription>This token will be locked in the gift card NFT.</FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
              />
      ...
      <Dialog open={isPurchased} onOpenChange={handleClose}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Your gift card NFT has been minted and added to your wallet.</DialogDescription>
          </DialogHeader>
          <div className="mx-auto max-w-[280px]">
            <div className="overflow-hidden rounded-lg border border-purple-500/50 bg-gradient-to-br from-purple-950 to-slate-900 shadow-lg">
              <div className="relative aspect-square bg-black/20">
                <Image src="/placeholder.svg?height=300&width=300" alt="Gift Card NFT" fill className="object-cover" />
              </div>
              <div className="p-4 space-y-2">
                <div className="flex items-center justify-between">
                  <span className="font-medium">{currentToken} Gift Card</span>
                  <span className="text-xs bg-purple-500/20 text-purple-300 px-2 py-1 rounded-full">#1234</span>
                </div>
                <div className="flex items-center justify-between">
          ...
          <DialogFooter className="flex flex-col sm:flex-row gap-2">
            <Button variant="outline" className="border-purple-800/50" onClick={handleClose}>
              Close
            </Button>
