...
use starknet::storage::StoragePathEntry;
    ...
    impl NiftImpl of INift<ContractState> {
        ...
        fn redeem_gift_card(
            ...
            // validate gift card
            self.assert_gift_is_available(
                token_id
            );
            // validate caller
            self.assert_non_zero_caller(caller);
            self.assert_is_gift_card_owner(
                token_id,
                caller
            );
            // update gift card
        ...
        fn get_gift_card_info(
            self: @ContractState,
            token_id: u256
        ) -> Gift {
            self.gifts.read(token_id)
        }
        fn get_user_purchased_gifts(
            self: @ContractState,
            user: ContractAddress
        ) -> Array<Gift> {
    ...
    pub impl InternalImpl of InternalTrait{
        ...
        fn assert_has_token_balance(
            ...
            assert(
                dispatcher.balance_of(caller) >= amount,
        ...
        fn assert_is_gift_card_owner(
            ...
            assert(
                erc721_dipatcher.owner_of(token_id) == caller,
                'Not Gift owner'
            )
        }
